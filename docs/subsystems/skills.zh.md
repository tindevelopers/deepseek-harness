...
`dsh-skill-bootstrap` 每个会话至多注入一次某个命名技能的完整主体，作为一条持久的用户角色消息，且仅在非平凡的多轮编程会话中注入。它是可选的：挂载该插件即启用，随附的预设不挂载它。闸门在首轮请求匹配 `programmingSignals` 模式时打开，或在会话到达 `escalateAtTurn`（默认 `2`）时无条件打开。监听器把注入追加到被接受步骤的消息批次末尾，并跳过可见表面已携带 `skill-bootstrap` 消息的步骤，因此主体只落地一次，仅在压缩遮蔽它时重新出现。加载以调用代理为作用域复用 `ctx.skills.get()`，因此注入的主体与 `skill` 工具返回的一致，并遵守 `isModelInvocable`。默认引导 `using-superpowers`；`skillName`、`escalateAtTurn` 与 `programmingSignals` 正则列表是配置面，在生成的[配置目录](../config-catalog.zh.md)中校验并记录。
