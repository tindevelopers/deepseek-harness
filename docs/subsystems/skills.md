...
`dsh-skill-bootstrap` injects one named skill's complete body as a durable user-role message once per session, gated so a trivial one-shot request never receives it. It is opt-in: mounting the plugin is the enablement, and the shipped presets leave it off. The gate opens at the first turn only when the request matches a `programmingSignals` pattern; any session that survives to `escalateAtTurn` (default `2`) opens it unconditionally. The listener appends the injection to the step's accepted message batch and skips a step whose visible surface already carries a `skill-bootstrap` message, so the body lands once and reappears only when compaction shadows it. Loading reuses `ctx.skills.get()` with the calling agent as scope, so the injected body is the same one the `skill` tool would return and honors `isModelInvocable`. The defaults bootstrap `using-superpowers`; `skillName`, `escalateAtTurn`, and the `programmingSignals` regex list are the configuration surface, validated and documented in the generated [config catalog](../config-catalog.md).
